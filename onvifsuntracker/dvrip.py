...
class DVRIPCam(object):
    ...
    def send(self, msg, data=None):
        ...
        if data:
            if hasattr(data, "__iter__"):
                data = bytes(json.dumps(data, ensure_ascii=True, sort_keys=True, indent=4), "utf-8")
            payload = struct.pack(
                "BB2xII2xHI", #"BB2xII2xHI",
                255,
                1, # KB changed to 1. Protocol version?
                self.session,
                self.packet_count,
                msg,
                len(data) + 2,
    ...
    def goToPositition(self):
        return self.setFlag(self.QCODES["GoToPosition"], "GoToPosition")

    def fishEyePlatform(self):
        return self.setFlag(self.QCODES["GoToPosition"],"FishEyePlatform")


    def ptz(self, cmd, step=5, preset=65535, ch=0, pattern="SetBegin", bottom=0, left=0, right=0, top=0):
        CMDS = [
            "DirectionUp",
            "DirectionDown",
            "DirectionLeft",
